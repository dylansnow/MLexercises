...
vector<tuple<T,U>> closestpair(vector<tuple<T,U>> P, vector<tuple<T,U>> X, vector<tuple<T,U>> Y) {
    if(P.size() <= 3) {
        ...
        Xl = Yl = Pl;
        Xr = Yr = Pr;
        sort(begin(Yl), end(Yl), [](auto const &t1, auto const &t2) { return get<1>(t1) < get<1>(t2); });
        sort(begin(Yr), end(Yr), [](auto const &t1, auto const &t2) { return get<1>(t1) < get<1>(t2); });
...
int main() {
    ...
    cout << endl;
    cout << "Euclidean distance: " << euclidean(tuple<double, double> (1.0,1.0),
        tuple<double, double> (2.0,2.0)) << endl;

       vector<tuple<double, double>> tuples = { make_tuple(1.0 , 1.0), make_tuple(3.0, 3.0), make_tuple(3.0, 11.0), make_tuple(8.0, 8.0) };
    vector<tuple<double, double>> result = closestpair(tuples);
    
    cout << "Euclidean distance result: " << euclidean(result[0], result[1]) << endl;
